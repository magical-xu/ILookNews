...
public class ChannelAdapter extends BaseMultiItemQuickAdapter<Channel, BaseViewHolder> {
  ...
  protected void convert(final BaseViewHolder baseViewHolder, final Channel channel) {
    switch (baseViewHolder.getItemViewType()) {
      case Channel.TYPE_MY:
        onBindMyChannelTitle(baseViewHolder, channel);
        break;
      ...
      case Channel.TYPE_MY_CHANNEL:
        onBindMyChannel(baseViewHolder, channel);
        break;
      case Channel.TYPE_OTHER_CHANNEL:
        onBindOtherChannel(baseViewHolder, channel);
        break;
    }
  }
   ...
   * 我的频道 标题
   */
  private void onBindMyChannelTitle(final BaseViewHolder baseViewHolder, Channel channel) {
    //赋值，以便之后修改文字
   ...
   * 推荐频道列表
   */
  private void onBindOtherChannel(final BaseViewHolder baseViewHolder, final Channel channel) {

    baseViewHolder.setText(R.id.tvChannel, channel.title).setVisible(R.id.ivDelete, false);
    baseViewHolder.setOnClickListener(R.id.tvChannel, new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        ...
        if (mRecyclerView.indexOfChild(targetView) >= 0 && myLastPosition != -1) {
          ...
          if (myChannelSize % spanCount == 0) {
            ...
            View lastFourthView =
                mRecyclerView.getLayoutManager().findViewByPosition(getMyLastPosition() - 3);
   ...
   * 我的频道列表
   */
  private void onBindMyChannel(final BaseViewHolder baseViewHolder, final Channel channel) {

    final boolean access = channel.access;
    if (access) {
      baseViewHolder.setTextColor(R.id.tvChannel,
          ILookApplication.getAppContext().getResources().getColor(R.color.three_text_color));
    } else {
      baseViewHolder.setTextColor(R.id.tvChannel,
          ILookApplication.getAppContext().getResources().getColor(R.color.two_text_color));
    }

    //编辑模式就显示删除按钮
    baseViewHolder.setVisible(R.id.ivDelete, mIsEdit);

    baseViewHolder.setOnLongClickListener(R.id.rlItemView, new View.OnLongClickListener() {
      @Override
      public boolean onLongClick(View v) {

        //add by magical to judge default tag
        if (access) {
          return false;
        }
        ...
        if (onChannelDragListener != null) onChannelDragListener.onStarDrag(baseViewHolder);
        return true;
      }
    }).addOnClickListener(R.id.rlItemView);
        ...
        .setOnClickListener(R.id.ivDelete, new View.OnClickListener() {
          ...
          public void onClick(View v) {
            ...
            if (mIsEdit) {
              ...
              //获取到目标View
              View targetView =
                  mRecyclerView.getLayoutManager().findViewByPosition(otherFirstPosition);
              //获取当前需要移动的View
              View currentView =
                  mRecyclerView.getLayoutManager().findViewByPosition(currentPosition);
              // 如果targetView不在屏幕内,则indexOfChild为-1  此时不需要添加动画,因为此时notifyItemMoved自带一个向目标移动的动画
              // 如果在屏幕内,则添加一个位移动画
              if (mRecyclerView.indexOfChild(targetView) >= 0 && otherFirstPosition != -1) {
                ...
                if (onChannelDragListener != null) {
                  onChannelDragListener.onMoveToOtherChannel(currentPosition,
   ...
   * 兼容 BRVAH 版本变动
   */
  private int getViewHolderPosition(BaseViewHolder baseViewHolder) {
  ...
  private void startAnimation(final View currentView, int targetX, int targetY) {
    ...
    TranslateAnimation animator =
        getTranslateAnimator(targetX - currentView.getLeft(), targetY - currentView.getTop());
  ...
  private ImageView addMirrorView(ViewGroup parent, View view) {
    ...
    parent.addView(mirrorView,
        params);//在RecyclerView的Parent添加我们的镜像View，parent要是FrameLayout这样才可以放到那个坐标点
    return mirrorView;
  }
  ...
  private TranslateAnimation getTranslateAnimator(float targetX, float targetY) {
    TranslateAnimation translateAnimation =
        new TranslateAnimation(Animation.RELATIVE_TO_SELF, 0f, Animation.ABSOLUTE, targetX,
  ...
  private void startEditMode(boolean isEdit) {
    ...
    for (int i = 0; i < visibleChildCount; i++) {
      ...
      //add by magical judge is default and no display delete icon
      Channel channel = mData.get(i);
      boolean access = channel.access;

      ImageView imgEdit = view.findViewById(R.id.ivDelete);
      if (imgEdit != null) {
        boolean isVis = imgEdit.getTag() == null ? false : (boolean) imgEdit.getTag();

        //modify by magical : add judge flag 'access'
        imgEdit.setVisibility(isVis && isEdit && !access ? View.VISIBLE : View.INVISIBLE);
      }
    }
  }
}
